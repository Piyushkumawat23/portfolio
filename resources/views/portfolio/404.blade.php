<!DOCTYPE html>
<html class="no-js" lang="en">

<head>
    <!-- Meta Tags -->
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="author" content="Laralink">
    <!-- Favicon Icon -->
    <link rel="icon" href="public/assets/img/favicon.png">
    <!-- Site Title -->
    <title> - Page Not Found</title>
    <link rel="stylesheet" href="public/assets/css/plugins/bootstrap.min.css">
    <link rel="stylesheet" href="public/assets/css/plugins/fontawesome.min.css">
    <link rel="stylesheet" href="public/assets/css/plugins/odometer-theme-default.css">
    <link rel="stylesheet" href="public/assets/css/plugins/select2.min.css">
    <link rel="stylesheet" href="public/assets/css/plugins/animate.css">
    <link rel="stylesheet" href="public/assets/css/style.css">
</head>

<body>
    <div class="cs_preloader cs_center">
        <div class="cs_preloader_in"></div>
    </div>

    <section class="cs_not_found cs_style_1 cs_center position-relative">
        <div class="position-absolute cs_not_found_shape_1">
            <img src="public/assets/img/about_shape_2.svg" alt="">
        </div>
        <div class="container">
            <div class=" text-center">
                <img class="cs_img" src="public/assets/img/not_found_1.svg" alt="errorPageOneImage">
                <h1 class="cs_title cs_font_48 cs_semi_bold">Sorry! Page did <span class="cs_accent_color">not found</span> </h1>
                <a class="cs_btn cs_style_1" href="{{ url('/') }}"><span>Back To Home</span></a>
            </div>
        </div>
    </section>

    <!-- Script -->
    <script src="public/assets/js/plugins/jquery-3.6.0.min.js"></script>
    <script src="public/assets/js/plugins/gsap.min.js"></script>
    <script src="public/assets/js/plugins/isotope.pkg.min.js"></script>
    <script src="public/assets/js/plugins/odometer.min.js"></script>
    <script src="public/assets/js/plugins/wow.min.js"></script>
    <script src="public/assets/js/main.js"></script>
</body>

</html>